...
async function scrapeHeyElsa(period, maxPages = 20) {
  let browser;
  try {
    console.log(`[HeyElsa ${period}] Starting...`);
    ...
    for (let currentPage = 1; currentPage <= maxPages; currentPage++) {
      ...
      console.log(`[HeyElsa ${period}] ✅ Page ${currentPage}: ${usersOnPage.length} (total: ${allUsers.length})`);
    ...
    await browser.close();
    console.log(`[HeyElsa ${period}] ✅ Complete: ${allUsers.length}`);
    return allUsers.length > 0 ? allUsers : null;
  } catch (e) {
    if (browser) await browser.close();
    console.error(`[HeyElsa ${period}] ❌`, e.message);
    return null;
  }
}

module.exports = { scrapeHeyElsa };

...
export default function PairingClient({
  ...
  return (
    <>
      {/* Blocks the page until the student claims their name */}
      {needsSetup && (
      ...
      <div className="min-h-screen p-6 max-w-3xl mx-auto">
          ...
          {myPair ? (
            <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl flex items-center gap-3">
              ...
              <span className="text-green-800 dark:text-green-300">
                You are paired with{' '}
                <strong>{myPartner?.full_name || myPartner?.email}</strong>
              </span>
            </div>
          ) : (
            <div className="mt-4 flex items-center gap-2 text-sm">
              ...
              {windowOpen ? (
                <span className="text-amber-600 dark:text-amber-400">
                  Window closes{' '}
                  {endDate ? new Date(endDate).toLocaleDateString() : 'soon'}
                </span>
              ) : (
        ...
        <div className="space-y-3">
          ...
          {profiles.map(profile => {
            ...
            const isPaired  = !!myPair;

            return (
              <div
                key={profile.id}
                className="flex items-center justify-between p-4 bg-white dark:bg-zinc-900
                           border border-zinc-200 dark:border-zinc-800 rounded-xl hover:shadow-sm transition"
              >

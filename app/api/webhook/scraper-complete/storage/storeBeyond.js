...
export async function storeBeyond(users, period, days, snapshotId) {
  ...
  const { error } = await supabase
    .from('beyond_leaderboard')
    .insert(records)
    .select();

  if (error) throw error;

  await updateCache('beyond', days, fetchedAt, records.length, snapshotId);

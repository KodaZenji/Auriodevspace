...
export async function storeMindoshare(users) {
  ...
  console.log(`[Mindoshare] Mapped sample:`, JSON.stringify(records[0]));
  ...
  if (error) {
    console.error('[Mindoshare] Insert error:', error);
    throw error;
  }

  await updateCache('PerceptronNTWK', 0, fetchedAt, records.length);
  
  console.log(`[Mindoshare] ✅ Stored ${records.length} users`);
}
